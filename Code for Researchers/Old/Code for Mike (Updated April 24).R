...
post_SIR_sample = SIR_algorithm(Npostsamp = Npostsamp, cum_weights = imp_vals$cum_weights, 
...
prior_psi_dens = prior_psi$density
...
post_psi_dens = diff(post_psi_cdf)
post_psi_dens = post_psi_dens / delta_psi
post_psi_dens = c(0, post_psi_dens) # needs to be of length (nbreaks - 1) not (nbreaks - 2)
...
post_psi_dens = average_vector_values(post_psi_dens, 3)
...
par(mfrow = (c(1, 3)))
plot(mids, prior_psi_dens, type = "l", xlab = "prior", ylab = "density", col = "red")
plot(mids, post_psi_dens, type = "l", xlab = "post", ylab = "density", col = "blue")
plot(mids, RB_psi, type = "l", xlab = "?", ylab = "RB", col = "green")

##########################################
# estimate for the relative belief ratio
RBest = mids[which.max(RB_psi)]

##########################################
# estimating plausible region
plaus_region = ifelse(RB_psi > 1, mids, 0)
...
##########################################
# COMMENTS: PLEASE PAY ATTENTION TO BELOW.
# getting the posterior content of the plausible region
# this is what Mike suggested, but I feel like it is wrong.
plaus_content = 0
for(i in 1:length(mids)){
  plaus_content = plaus_content + (plaus_region[i] * post_psi_dens[i])
}
...
plaus_content = 0
for(i in 1:length(plaus_region)){
  if(plaus_region[i] != 0){
    plaus_content = plaus_content + (post_psi_dens[i] * delta_psi) 
  }
}

##########################################
# access hypothesis psi = psi0
psio = -2
psio_index = which.min(abs(mids - psio))

# evidence for psio
evidence_psio = RB_psi[psio_index]
evidence_psio

##########################################
# compute strength
indices = which(RB_psi <= evidence_psio)
# Compute the strength
strength_psio = sum(post_psi_dens[indices]) * delta_psi
strength_psio
