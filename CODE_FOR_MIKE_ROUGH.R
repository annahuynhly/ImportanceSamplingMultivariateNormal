...
prior_content_vals = prior_content(N, p, m = 400, mu = sample_prior_vals$mu_matrix, 
...
# PUT THE LAMBDA FUNCTION IN THE MAIN ONE AS WELL IF THIS WORKS
...
# denote the function of h - user needs to manually change this
h = function(mu, xi){
  # user should modify this. for now, we have a degenerate function
  return(mu)
}

k_lambda0 = function(p, mu, xi, alpha01, alpha02, mu0, lambda0){
  
  p = length(alpha01)
  sigmaii = c()
  for(i in 1:p){
    sigmaii = c(sigmaii, rgamma(1, shape = alpha01[i], scale = alpha02[i]))
  }
  
  Lambda0 = diag(lambda0)
  newlambda0 = max(lambda0)^(-2)
  inv_Lambda0 = find_inverse_alt(Lambda0)
  #x1 = inv_Lambda0 %*% xi %*% inv_Lambda0 
  x1 = exp(-(1/2) * t(mu - mu0) %*% (inv_Lambda0 %*% xi %*% inv_Lambda0 - newlambda0 * xi) * (mu - mu0))
  x3 = exp(-alpha02/sigmaii)
  x2 = 1
  for(i in 1:p){
    x2 = x2 * (1/sigmaii[i])^(alpha01[i] + (p+1)/2)
  }
  return(x1 * x2 * x3)
}

importance_sampler = function(N, p, mu, xi, alpha01, alpha02, mu0, lambda0){
  num = rep(0, p)
  denom = rep(0, p)
  for(i in 1:N){
    num = num + h(mu[i, ], xi[,,i]) * k(p, mu[i,], xi[,,i], alpha01, alpha02, mu0, lambda0)
    denom = denom + k_lambda0(p, mu[i,], xi[,,i], alpha01, alpha02, mu0, lambda0)
  }
  return(num/denom)
}
...
#post_content_vals = posterior_content(N, p, m = 400, post_mu, post_xi, test_weights)
...
# values for the importance sampler
importance_sampler(N = 1000, p, mu = post_vals$mu, xi = post_vals$xi, 
                   alpha01, alpha02, mu0, lambda0)

##################################################
# GRAPHS FOR THE POSTERIOR                       #
##################################################

col_num = 1 # the column number
smooth_num = 3 #number of points to smooth for the posterior and relative belief ratio

# Below are cosmetic changes for the graph.
type = "prior"
colour_choice = c("#3172f5", "#3172f5")
transparency = 0.1
delta = 0.05
lty_type = 2

mu_values = post_vals$mu[,col_num]
